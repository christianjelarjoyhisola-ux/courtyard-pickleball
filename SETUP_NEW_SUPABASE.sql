...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'open_play_receipt_status_check') THEN
    ALTER TABLE public.open_play_registrations
...
-- Do not create a broad "bucket_id <> 'receipts'" policy here: PostgreSQL RLS
-- policies are permissive, so that predicate would grant anon/authenticated
-- access to every other current or future bucket. With no direct receipts
-- policy, anon/authenticated are denied and the service role remains the only
-- receipt-object access path.
...
VALUES
  ('venue_name',    'CourtYard Pickleball'),
  ('open_time',     '6'),
  ('close_time',    '22'),
  ('open_hour',     '6'),
  ('close_hour',    '22'),
  ...
  ('gcash_merchant_number', '0935 935 5010'),
  ('gcash_merchant_name', 'CourtYard Pickleball'),
...
-- Kept in sync with supabase/migrations/20260713_secure_receipt_verification.sql.
-- ============================================================
...
-- Transaction-safe automatic GCash approval for strict zero-flag OCR results.
...
-- Keep the numeric session hours authoritative while also storing a friendly
